...
### 3-1. 카카오톡 공유(친구 초대 링크) — 선택

설정 화면의 "카카오톡으로 친구 초대"는 실제 카카오톡 공유창(Kakao SDK `Kakao.Share`)을 여는데, 이건 별도의 **JavaScript 키**가 필요합니다:

1. 같은 Kakao Developers 앱의 **앱 키 → JavaScript 키**를 복사합니다.
2. **제품 설정 → 카카오톡 공유**를 활성화합니다.
3. **플랫폼 → Web**에 배포 도메인(과 `http://localhost:3000`)을 등록합니다.
4. `.env.local`에 추가합니다:

   ```
   NEXT_PUBLIC_KAKAO_JS_KEY=abcd1234...
   ```

이 키가 없어도 초대 링크 자체(생성/공유/수락)는 정상 동작합니다 — 카카오톡 공유창 대신 브라우저 기본 공유창이나 클립보드 복사로 대체될 뿐입니다.
...
- **그룹 멤버 초대**: "내 친구" 목록에 있고 실제로 앱을 쓰는 사람만 그룹에 실시간 연동됩니다. 아직 앱을 쓰지 않는 사람은 이름만 저장된 "초대 대기중" 상태로 남습니다 — `/invite/[token]` 링크로 먼저 진짜 친구가 되면 이 상태가 풀립니다.

### 친구 추가 (구현됨)

설정 → "카카오톡으로 친구 초대"에서 1회용 링크(`friend_invites` 테이블, `supabase/migrations/010_friend_invites.sql`)를 만들어 카카오톡(또는 기본 공유창/클립보드)으로 보냅니다. 상대가 링크를 열어 카카오 로그인 후 수락하면 `accept_friend_invite()` RPC가 양쪽을 실제 프로필로 `friends`에 연결합니다. "이름으로 직접 추가"는 앱을 아직 안 쓰는 사람을 위한 이름만 저장하는 보조 수단으로 남겨뒀습니다.
